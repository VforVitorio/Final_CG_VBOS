...
from OpenGL.GL import *  # Se necesita para activar las luces y desactivarlas

# Bandera que indica si el botón izquierdo está presionado
...
def procesar_eventos_raton(evento, camara):
    ...
    # Verifica si la rueda del ratón se ha movido
        ...
        camara.ajustar_radio(-evento.y * SENSIBILIDAD_ZOOM *
                             INVERTIR_CONTROLES, RADIO_MIN, RADIO_MAX)
...
def consultar_estado_teclado(camara, delta_time):
    ...
    # Contro """Procesa los eventos de teclado y ratón."""
    # ... código exisl de rotación horizontal (yaw) hacia la derecha.
    if keys[K_RIGHT]:
        camara.ajustar_yaw(velocidad_rotacion)
    ...
    # CONTROL DE LAS LUCES, SEGUN 1 2 Y 3
    # Control de luces
    if keys[K_1]:  # Luz 1 encendida, Luz 2 apagada
        glEnable(GL_LIGHT0)
        glDisable(GL_LIGHT1)

    if keys[K_2]:  # Luz 1 apagada, Luz 2 encendida
        glDisable(GL_LIGHT0)
        glEnable(GL_LIGHT1)

    if keys[K_3]:  # Ambas luces encendidas
        glEnable(GL_LIGHT0)
        glEnable(GL_LIGHT1)
